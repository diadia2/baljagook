...
=======
	...
	@RequestMapping("/map/regMymap.do")
	public String regMyMap(@RequestParam("title") String title, @RequestParam("content") String content,
		...
		for (int i = 0; i < markerlatArr.length; i++) {
			...
			// �ش� ��Ŀ�� ��ǥ�� regcoordinates�� ��ǥ�� ��ġ�ϸ� regcoordinatesidx��
			// �����ͼ� t_checkpoint�� ����
			for (int j = 0; j < regList.size(); j++) {
				if (regList.get(j).getLat().equals(checkPointLat) && regList.get(j).getLon().equals(checkPointLon)) {
					CheckpointVO checkpointVO = new CheckpointVO();
	...
	@RequestMapping("/map/planMymap.do")
	public String planMymap(@RequestParam("title") String title, @RequestParam("content") String content,
			HttpServletRequest request) throws ParseException {
	...
	public String searchList(@RequestParam("searchtext") String searchtext, @RequestParam("moreCount") int moreCount,
		...
		List<MymapVO> mymapList = mymapService.selectMymapList(searchtext, 4 * moreCount);// 5*1
																							// 더보기
																							// 누르면
																							// 5*2
																							// 5*3
		...
		HashMap<Integer, Boolean> likeAlreadyChecked = new HashMap<Integer, Boolean>();
		// like 수 초기화
>>>>>>> 1aefc591df9e737d5449d51ec6b1e61838fc5f42
		int likeCnt = 0;
...
=======
	...
	@RequestMapping("/map/getFavoritePlace.do")
	@ResponseBody
	public String getFavoritePlace(@RequestParam("idx") String idx, @RequestParam("placename") String placename) {
